...
export interface ExampleItem {
  ...
  beforeWebpRetina: string;
  afterWebpRetina: string;
  width: number;
  height: number;
}
    ...
    <figure data-example-compare>
      <div className="border-border/80 bg-muted relative overflow-hidden rounded-xl border shadow-[0_1px_2px_rgba(0,0,0,0.04),0_10px_24px_-14px_rgba(0,0,0,0.15)]">
        <picture>
          <source
            type="image/webp"
            srcSet={`${item.afterWebpSmall} 480w, ${item.afterWebpMedium} 640w, ${item.afterWebpLarge} 1024w, ${item.afterWebpRetina} 1152w, ${item.afterWebp} ${item.width}w`}
        ...
        >
          <picture>
            <source
              type="image/webp"
              srcSet={`${item.beforeWebpSmall} 480w, ${item.beforeWebpMedium} 640w, ${item.beforeWebpLarge} 1024w, ${item.beforeWebpRetina} 1152w, ${item.beforeWebp} ${item.width}w`}
              sizes="(min-width: 1024px) 640px, (min-width: 640px) calc(100vw - 64px), calc(100vw - 34px)"
            />
            <img
              src={item.before}
