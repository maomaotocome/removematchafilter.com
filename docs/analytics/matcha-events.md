...
- **Production status:** Runtime delivery is default-deny; not approved for
  production opt-in until consent and policy review is complete
...
- Setting consent to `denied` or `unknown` clears the bounded in-memory queue
  immediately. Consent is not persisted by this helper.
...
The production public configuration returned a GA measurement ID during the
2026-08-16 review, and Search Console shows visitors from the EU, UK, Canada,
Philippines, and other regions. No consent-mode or consent-management component
was found in the application. The English privacy policy currently describes
analytics as aggregate page views/referrers and says adjustments are not
recorded; the Chinese policy does not contain an equivalent specific analytics
disclosure.

The product-event runtime therefore defaults to no collection. It may be tested
locally, but production consent must not be granted until the owner completes
an applicable-region legal/privacy review and, where required, implements a
reviewed consent UI/Google Consent Mode and updates both privacy-policy
languages to describe these bounded product events accurately. Existing GA
pageview behaviour is outside this P0 product-event slice.
