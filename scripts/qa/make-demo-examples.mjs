...
try {
  ...
  for (const job of JOBS) {
    ...
    const encoded = await page.evaluate(() => {
      ...
      const retinaWidth = 1152;
      const retinaHeight = Math.round((height / width) * retinaWidth);
    ...
    writeFileSync(
      `${OUT}/${job.slug}-before-1152.webp`,
      Buffer.from(encoded.beforeWebpRetina, 'base64')
    );
    writeFileSync(
      `${OUT}/${job.slug}-after-1152.webp`,
      Buffer.from(encoded.afterWebpRetina, 'base64')
    );
