...
const RETIRED_TEMPLATE_SURFACES = [
  ['/pricing?utm_source=seo-qa', '/'],
  ['/blog', '/'],
  ['/blog/what-is-shipany', '/'],
  ['/zh/pricing', '/zh'],
  ['/zh/blog/blocks-vs-components', '/zh'],
];
...
function validOptionalWebPageDates(webPage) {
  const published = webPage?.datePublished;
  const modified = webPage?.dateModified;
  if (!published && !modified) return true;
  if (!published || !modified) return false;
  if (
    !/^\d{4}-\d{2}-\d{2}$/.test(published) ||
    !/^\d{4}-\d{2}-\d{2}$/.test(modified)
  ) {
    return false;
  }
  const publishedAt = Date.parse(`${published}T00:00:00Z`);
  const modifiedAt = Date.parse(`${modified}T00:00:00Z`);
  const endOfToday = Date.now() + 24 * 60 * 60 * 1000;
  return (
    Number.isFinite(publishedAt) &&
    Number.isFinite(modifiedAt) &&
    publishedAt <= modifiedAt &&
    modifiedAt <= endOfToday
  );
}
  ...
  for (const target of TARGETS) {
    ...
    for (const [header, expected] of Object.entries(SECURITY_HEADERS)) {
      if (response.headers.get(header) !== expected) {
        fail(
          `${header} is ${response.headers.get(header) ?? 'missing'}; expected ${expected}`
        );
      }
    }
    ...
    if (target.path === '/') {
      ...
      if (!validOptionalWebPageDates(webPage)) {
        fail(
          'WebPage dates must be paired, valid, chronological, and not future'
        );
      }
...
if (
  ...
  fail('sitemap missing the Google image namespace');
}
const sitemapImageLocations = new Set(
...
for (const scene of [
  ...
  for (const state of ['before', 'after']) {
    const expected = `${canonicalOrigin}/imgs/examples/photo-${scene}-${state}.jpg`;
    if (!sitemapImageLocations.has(expected)) {
      fail(`image sitemap missing ${expected}`);
    }
  }
}
...
for (const [path, expectedPath] of RETIRED_TEMPLATE_SURFACES) {
  const response = await fetch(`${BASE}${path}`, { redirect: 'manual' });
  const location = response.headers.get('location');
  const redirectUrl = location ? new URL(location, ORIGIN) : null;
  console.log(`\n=== ${path} (retired template surface) ===`);
  ...
  if (
    ...
    redirectUrl?.searchParams.get('utm_source') !== 'seo-qa'
  ) {
    fail('retired-surface redirect did not preserve the query string');
  }
}
...
console.log('\n=== llms.txt and llms-full.txt (GEO truth) ===');
if (llmsResponse.status !== 200 || llmsFullResponse.status !== 200) {
  fail(`llms responses are ${llmsResponse.status}/${llmsFullResponse.status}`);
}
for (const output of [llms, llmsFull]) {
  if (/shipany|blocks vs components|## blog posts/i.test(output)) {
    fail('llms output still exposes unrelated template content');
  }
  for (const path of SITEMAP_PATHS) {
    const expectedUrl = `${canonicalOrigin}${path}`;
    if (!output.includes(expectedUrl)) {
      fail(`llms output missing ${expectedUrl}`);
    }
  }
}
if (llmsFull.length < llms.length * 2) {
  fail('llms-full.txt is not substantially fuller than llms.txt');
}
for (const fact of [
  'does not require an account',
  'cannot recover exact source pixels',
  'load only after the visitor explicitly allows analytics',
]) {
  if (!llmsFull.includes(fact)) fail(`llms-full.txt missing fact: ${fact}`);
}
...
for (const [path, expectedPath] of [
  ...
  ['/About?utm_source=seo-qa', '/about'],
  [
    '/Remove-Matcha-Filter-Tiktok?utm_source=seo-qa',
    '/remove-matcha-filter-tiktok',
  ],
  [
    '/REMOVE-MATCHA-FILTER-CAPCUT?utm_source=seo-qa',
